...
```
```{r getURLs, echo=FALSE}
# ptm<-proc.time()
# trainUrl <- "http://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv"
# testUrl <- "http://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv"
# getURLS.time<-proc.time()-ptm
```
...
model.fit2<-randomForest(classe ~ ., data=strain1, importance = TRUE)
...
ptm<-proc.time()
set.seed(3383)
predDF<-data.frame(predict1, predict2, predict3, classe=stest1$classe)
model.fit4<-train(classe ~., method="gam", data=predDF)
predict4<-predict(model.fit4, predDF)
cmp4<-confusionMatrix(predict4, predDF$classe)

mfit4time<-proc.time()-ptm
```
```{r t4_table, echo=FALSE, results='asis'}
t4<-cmp4$table
cmp4$overall["Accuracy"]
```

```{r timers, echo=FALSE}

times<-as.data.frame(rbind("rpart"=mfit1time[1:3], "randomForest" = mfit2time[1:3], "gbm"=mfit3time[1:3], "combined"=mfit4time[1:3]))

```
##Compare all 4 Models for Accuracy and Compare Processing Times
```{r comparison, echo=FALSE, warning=FALSE, message=FALSE}
modeltable<-as.data.frame(rbind("rpart" = cmp1$overall["Accuracy"], "random forest"=cmp2$overall["Accuracy"], "gradient boosting"=cmp3$overall["Accuracy"], "combined"=cmp4$overall["Accuracy"]))
...
Model 2, the random forest model has the highest accuracy.  Applied to our large training set vs. testing set we get the following predictions:
```{r echo=FALSE}
